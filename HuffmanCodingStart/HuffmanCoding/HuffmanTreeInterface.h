...
using namespace std;
 ...
 public:
	...
	*/
	...
	/**
	deletes all nose of the tree. Uses postorder traversal.
	...
	*/
	...
	/**
	decodes the string into a a series of chars 
	based on the huffman tree. The bits in eaach char should 
	...
	*/
	...
	@param uncompressedFileName -the file to uncompress
	@param compressedFileName -the file to store the decoded version in. 
	
	@return no return value
	*/
	virtual void uncompressFile(string compressedFileName, string uncompressedFileName) = 0;	

	
	/**
	encodes the contents of the compressedfile
	and saves its decompressed form in the target file
	file should contain valid huffman prefix codes
	in the form of binary (NOT text values 0 and 1)

	@param compressedFileName -the file to decode
	@param uncompressedFileName -the file to store
	@param rebuild tree before doing compression

	@return no return value
	*/
	virtual void compressFile(string compressedFileName, string uncompressedFileName, bool buildTree = true) = 0;
